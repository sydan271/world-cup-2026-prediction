      ...
      for (size_t i = 0; i < current_round.size(); i += 2) {
         ...
         if (res == 2) {
            ...
            if (current_round.size() == 2)
               runner_up = current_round[i + 1].name;
         } else {
            next_round.push_back(current_round[i + 1]);
            if (current_round.size() == 2)
               runner_up = current_round[i].name;
         }
      }
...
int main() {
   ...
   std::cout << "----------------------------------------------------------------\n";
   std::cout << std::left << std::setw(18) << "TEAM" << std::setw(15) << "WIN CHAMPION" << std::setw(15)
             << "RUNNER-UP"
             << "MAKE FINAL\n";
   std::cout << "----------------------------------------------------------------\n";
   ...
   for (const auto &result : results) {
      ...
      if (champ_prob >= 0.01) { // Show any team that has at least a 0.01% chance
         std::cout << std::left << std::setw(18) << result.first << std::fixed << std::setprecision(3)
                   << champ_prob << "%" << std::setw(8) << " " << runner_up_prob << "%" << std::setw(7) << " "
                   << make_final_prob << "%\n";
      }
   }
   ...
   std::cout << "\n\033[1;33m****************************************************************\033[0m\n";
   std::cout << "\033[1;36m🌟  JUST KIDDING HAHA, THE REAL WINNER IS ARGENTINAAAAA!  🌟\033[0m\n";
   std::cout << "\033[1;34m🇦🇷  MESSI IS GOING TO COOK! VAMOS! 🇦🇷\033[0m\n";
